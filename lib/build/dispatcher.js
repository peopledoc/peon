...
class Dispatcher {
  ...
  async dispatch(eventType, payload) {
    ...
    let { cleanup, status, Build } = lookup()
    let loggerOpts = { module: `dispatcher/${repoConfig.name}` }

    if (!payload.head_commit) {
      logger.info(
        `enqueuing cleanup for ${repoConfig.refMode} ${repoConfig.ref}`,
        loggerOpts
      )

      this.queue.run(() =>
        cleanup.cleanup(repoConfig.name, repoConfig.refMode, repoConfig.ref)
      )
    } else {
      ...
      logger.info(`enqueuing build ${buildId}`, loggerOpts)
