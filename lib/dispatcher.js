...
class Dispatcher {
  ...
  async build(payload, repoConfig) {
    ...
    try {
      ...
      let destDir = resolve(publicDirectory, repoName)
  ...
  findRepositoryToHandle(eventType, payload) {
    ...
    if (repoName) {
      ...
      if (allowUnknownRepositories) {
        repoName = url.replace(/[^a-z0-9]/gi, '_')
        repoConfig = {
          branch: 'master',
          url
        }
    ...
    if (ref === `refs/heads/${repoConfig.branch}`) {
      return Object.assign({ name: repoName }, repoConfig)
    } else {
