...
class Dispatcher {
  ...
  findRepositoryToHandle(eventType, payload) {
    ...
    if (repoName) {
      ...
      let allow = false
      if (unknownRepositories && 'allowOrgs' in unknownRepositories) {
        for (let org of unknownRepositories.allowOrgs) {
          if (
            url.startsWith(`ssh://git@github.com/${org}/`)
            || url.startsWith(`git@github.com:${org}/`)
          ) {
            allow = true
            break
          }
